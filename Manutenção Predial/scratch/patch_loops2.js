...
// The getters in start_test_server.js are written with single backslashes in template literals.
// We need to double the backslashes so they are proper Regexes.
// I will just read the file and replace the whole block again.
...
if (startIdx !== -1 && endIdx !== -1) {
    ...
    code = code.substring(0, startIdx) + newBlock + code.substring(endIdx + 2);
    fs.writeFileSync('start_test_server.js', code, 'utf8');
    console.log("Replaced getterMatches with properly escaped RegExp template strings.");
} else {
