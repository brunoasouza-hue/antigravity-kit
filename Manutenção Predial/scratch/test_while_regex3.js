...
function compileConditionals(html) {
    ...
    do {
        ...
        const ifElseRegex = /<\?php\s+if\s*\(((?:(?!\?>)[\s\S])*?)\)\s*:\s*\?>((?:(?!<\?php\s+if)[\s\S])*?)<\?php\s+else\s*:\s*\?>((?:(?!<\?php\s+if)[\s\S])*?)<\?php\s+endif\s*;\s*\?>/gs;
        ...
        const ifRegex = /<\?php\s+if\s*\(((?:(?!\?>)[\s\S])*?)\)\s*:\s*\?>((?:(?!<\?php\s+if)[\s\S])*?)<\?php\s+endif\s*;\s*\?>/gs;
